...
from keras.layers import Dense, Activation, Dropout, Input,BatchNormalization, Lambda
#VAE
...
for ff in fliename:
  ...
  for k, (train, test) in enumerate(sfolder):
      ...
      x_train_ae = VAE_e.predict(kfold_x_train)
      x_test_ae = VAE_e.predict(kfold_x_test)
      ...
      df1.to_csv('VAE+SVM+KNN/'+str(file)+'_vae_train_'+str(k+1)+'.csv',index=False,encoding='utf-8')
      df2.to_csv('VAE+SVM+KNN/'+str(file)+'_vae_test_'+str(k+1)+'.csv',index=False,encoding='utf-8')
      ...
      test_ans.to_csv('VAE+SVM+KNN/'+str(file)+'_svm+vae_ans_'+str(k+1)+'.csv',index=False,encoding='utf-8')
      ...
      y_ans = pd.DataFrame(y_ans,columns = ["true"])
      test_ans = y_ans.merge(y_guess, how='left', left_index=True, right_index=True)
      test_ans.to_csv('VAE+SVM+KNN/'+str(file)+'_knn+vae_ans_'+str(k+1)+'.csv',index=False,encoding='utf-8')
  ...
  ansc45knn.to_csv('VAE+SVM+KNN/(ans)vae+SVM+KNN.csv',index=False,encoding='utf-8')