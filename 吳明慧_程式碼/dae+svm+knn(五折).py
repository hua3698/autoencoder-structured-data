...
for ff in fliename:
  ...
  for k, (train, test) in enumerate(sfolder):
      ...
      x_train_ae = ae_e.predict(x_train_noisy)
      x_test_ae = ae_e.predict(x_test_noisy)
      ...
      df1.to_csv('DAE+SVM+KNN2/' + str(file) + '_dae_train_' + str(k+1) + '.csv', index=False, encoding='utf-8')
      df2.to_csv('DAE+SVM+KNN2/' + str(file) + '_dae_test_' + str(k+1) + '.csv', index=False, encoding='utf-8')
      ...
      test_ans.to_csv('DAE+SVM+KNN2/'+str(file)+'_svm+dae_ans_'+str(k+1)+'.csv',index=False,encoding='utf-8')
      ...
      y_ans = pd.DataFrame(y_ans, columns = ["true"])
      test_ans = y_ans.merge(y_guess, how='left', left_index=True, right_index=True)
      test_ans.to_csv('DAE+SVM+KNN2/'+str(file)+'_knn+dae_ans_'+str(k+1)+'.csv',index=False,encoding='utf-8')
  ...
  ansc45knn.to_csv('DAE+SVM+KNN2/(ans)DAE+SVM+KNN.csv',index=False,encoding='utf-8')