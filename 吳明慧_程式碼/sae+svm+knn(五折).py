...
for ff in fliename:
  ...
  for k, (train, test) in enumerate(sfolder):
      ...
      df1.to_csv('SAE+SVM+KNN/'+str(file)+'_sae_train_'+str(k+1)+'.csv',index=False,encoding='utf-8')
      df2.to_csv('SAE+SVM+KNN/'+str(file)+'_sae_test_'+str(k+1)+'.csv',index=False,encoding='utf-8')
      ...
      test_ans.to_csv('SAE+SVM+KNN/'+str(file)+'_svm+sae_ans_'+str(k+1)+'.csv',index=False,encoding='utf-8')
      ...
      y_ans = pd.DataFrame(y_ans,columns = ["true"])
      test_ans = y_ans.merge(y_guess, how='left', left_index=True, right_index=True)
      test_ans.to_csv('SAE+SVM+KNN/'+str(file)+'_knn+sae_ans_'+str(k+1)+'.csv',index=False,encoding='utf-8')
  ...
  ansc45knn.to_csv('SAE+SVM+KNN/(ans)SAE+SVM+KNN.csv',index=False,encoding='utf-8')